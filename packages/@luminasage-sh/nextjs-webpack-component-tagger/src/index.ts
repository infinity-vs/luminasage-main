...
/**
 * A webpack loader that adds `data-luminasage-*` attributes to JSX elements.
 */
export default function luminasageTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                attr.name?.name === "data-luminasage-id",
            );
            if (alreadyTagged) return;

            // Build the luminasage ID
            const loc = node.loc?.start;
            if (!loc) return;
            const luminasageId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-luminasage-id="${luminasageId}" data-luminasage-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[luminasage-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[luminasage-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[luminasage-tagger] ERROR in ${this.resourcePath}:`, err);
