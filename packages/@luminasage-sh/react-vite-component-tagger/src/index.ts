 ...
 */
export default function luminasageTagger(): Plugin {
  return {
    name: "vite-plugin-luminasage-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-luminasage-id ───────────────
              ...
              console.warn(
                `[luminasage-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[luminasage-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
